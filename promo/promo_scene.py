"""eBrowser — production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eBrowser", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Secure, Fast, Extensible Lightweight Browser", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "C, WebKit, OpenSSL, SQLite".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "Military-Grade Sandboxing", "Every tab runs in an isolated container with seccomp filters and namespace separation", DUR["f1"]),
            ("02", "Zero-Trust Firewall", "Network-level filtering blocks malicious connections before they reach your browser", DUR["f2"]),
            ("03", "Anti-Fingerprinting Engine", "Randomizes canvas, WebGL, and font metrics to prevent cross-site tracking", DUR["f3"]),
        ]
        ...
        components = ["Tab Process", "Sandbox", "Firewall", "Render Engine", "Network Stack"]
        boxes = VGroup()
